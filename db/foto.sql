...
CREATE TABLE megusta (
  ...
  id_userfoto INT,
  FOREIGN KEY (id_user) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
  FOREIGN KEY (id_userfoto) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE chat (
  ...
  fecha_chat TIMESTAMP(0) NOT NULL,
  FOREIGN KEY (id_user_login) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
...
CREATE TABLE contenido (
  ...
  id_user INT,
  nombre text,
  imagecontenido VARCHAR(900),
  edad VARCHAR(3),
  precio text,
  descripcion text,
  fecha_contenido TIMESTAMP(0) NOT NULL,
  FOREIGN KEY (id_user) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
);

DROP TABLE IF EXISTS fotocontenido CASCADE;
CREATE TABLE fotocontenido (
  id_fotocontenido BIGSERIAL PRIMARY KEY,
  id_contenido INT,
  fotocontenido VARCHAR(900),
  codigo text,
  fecha_fotocontenido TIMESTAMP(0) NOT NULL,
  FOREIGN KEY (id_contenido) REFERENCES contenido(id_contenido) ON UPDATE CASCADE ON DELETE CASCADE,
);
